...
The package validates the session workspace before resolving the CLI. Relative paths resolve against `agent.session.header.cwd` or configured `workspaceRoot`, realpath to an existing directory, and must remain inside that root. Indexing rejects an existing output directory or graph whose canonical path escapes through a symlink or junction, while a missing output is permitted beneath the canonical workspace. Query operations require a regular readable contained graph and never accept a model-provided graph path. The subprocess environment sets `GRAPHIFY_QUERY_LOG_DISABLE=1` so model calls do not write Graphify's optional plaintext query log.

The CLI remains external. `binaryPath` defaults to `graphify`, and `binaryArgs` supports wrappers such as `uvx --from graphifyy graphify` without shell interpolation. Missing binaries fail before spawn with an install hint that omits host resolver details. The subprocess provider scrubs ambient credential-shaped variables; the plugin deliberately forwards no credentials. Timeouts and caller cancellation abort the spawned process tree, and the plugin awaits whole-tree exit before publishing an outcome. Collected stdout/stderr are byte-capped and returned with exit, signal, timeout, and truncation facts, while private collector spill paths remain internal.
...
The feature is safe to install without changing default profiles, and a configured deployment gets a narrow graph workflow: build/update the current workspace graph, then query/explain/path against that graph. Initial indexing defaults to Graphify's supported `--code-only --no-cluster` fast path and fixes `--out` to the workspace; updates fix the scan root to the workspace so reads and writes share one graph. Opting into semantic extraction requires a credential-free backend or a separately configured wrapper because Harness credentials are not inherited. The adapter does not parse Graphify's graph JSON into a Harness-native graph type; it newline-normalizes Graphify's CLI text and records bounded process facts beside it. Large initial indexes remain foreground tool calls and may need a wider deployment timeout or an out-of-agent Graphify run.

The shipped checks pin the boundary: unit tests cover canonical containment including junction escapes, missing paths, argv ordering, wrapper arguments, cancellation, deadlines, output caps, sensitive startup diagnostics, queue classification, missing graphs, query flags, non-zero exits, and deterministic text rendering; a Loader smoke boots the package through app-boot with a fake argv-only CLI; the invariant companion validates live Graphify tool-result records.
