...
| `maxLines` | *(未设置)* | 可选的行数触发器，灵感来自 pi agent 的双限截断：当合并文本的换行分隔行数超过此值时也会修剪，即使未达到 `thresholdChars`。以先触发者为准。省略则保持纯按字符修剪。 |

字符数以 Unicode 码点计，因此切片绝不会拆分 emoji 对，但多字符字素仍可能被切断。头部加标记加尾部之和必须不超过阈值，因此按字符触发的结果修剪时不会增长或重复改写。当仅 `maxLines` 在字符较少的内容上触发时，剪枝器在相同的头/尾预算内保留完整的前导与末尾行，并跳过任何太小而无法缩减到标记以下的结果，因此替换结果始终严格小于其输入。未知设置会在构造时拒绝插件。
