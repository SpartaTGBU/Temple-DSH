...
已记录的响应内容会追加到下一个请求，不会使其更早可复用前缀失效。未记录的传输元数据与用量计量不影响缓存标识。提示缓存默认开启：未设置 `cacheRetention` 的路由解析为 `short`，因为工具调用回合会在多次往返中复用同一前缀，而提供方的缓存读取成本仅为全新输入令牌的一小部分；设置 `cacheRetention: 'none'` 可使某个路由退出。
