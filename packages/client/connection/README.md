...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; in-process compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half owns the sole `/api` route, Fetch bridge, browser authentication, Host/Origin checks, and exact `GET`/`HEAD` route registry. Typert Gateway and feature packages can claim disjoint unary endpoint sets on that shared route, feature packages register non-JSON responses such as Session-log downloads, unclaimed requests return 404, and overlapping unary claims fail closed with an ambiguity error. Loopback hostname classification remains package-internal to the browser-facing Client state.
