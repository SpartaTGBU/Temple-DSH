...
Set `MEMPALACE_ENABLED=1` to enable this browser row together with the native provider, automatic memory consumer, and Host API. The section appears in Settings as **MemPalace** and offers filters for wing, room, and drawer text. Editing filters performs no requests; initial mount and explicit Refresh each read one current Host snapshot, and stale completions are ignored after refresh or unmount.
...
None, as this package registers a browser Settings section only and contributes nothing model-facing.
