...
提供方维护有界捕获队列，在超时或协议输出格式错误后重启 worker，在 TypeScript 边界再次应用召回和结果上限，并在 flush 与销毁时排空已接受捕获工作。它的 `inspectionSource()` 操作通过同一个 bridge 配置解析 palace、collection、存储后端与捕获 wing，而不打开或创建存储。子进程 stderr、自由格式提供方细节和凭据不会进入仪表盘或模型可见错误。
